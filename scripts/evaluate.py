...
def test_source_preference(model, tokenizer):
    ...
    for i, test in enumerate(test_cases, 1):
        ...
        results.append({
            "test": i,
            "prefers_primary": prefers_primary,
            "response": response
        })
...
def evaluate_on_validation_set(model, tokenizer, val_file: str):
    ...
    for i, ex in enumerate(val_data[:3], 1):
        ...
        prompt = ex['text'].split("User:")[1].split("Assistant:")[0].strip() if "User:" in ex['text'] else ex['text'][:100]
...
def main():
    ...
    parser.add_argument("--skip-source-test", action="store_true", help="Skip source preference test")
    parser.add_argument("--skip-distrust-test", action="store_true", help="Skip distrust behavior test")
    parser.add_argument("--skip-val-eval", action="store_true", help="Skip validation set evaluation")
