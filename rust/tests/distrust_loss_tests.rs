use your_ai_rs::distrust_loss::{empirical_distrust_loss, batch_empirical_distrust_loss, validate_inputs};
use mlx_rs::Array;
...
fn test_primary_source_high_loss() {
    ...
    assert!(value > 100.0, "Primary source should have high loss: {}", value);
}
...
fn test_modern_consensus_low_loss() {
    ...
    let value: f32 = result.item();
    assert!(value < 50.0, "Modern consensus should have low loss: {}", value);
}
...
fn test_batch_loss_mean_reduction() {
    ...
    let result = batch_empirical_distrust_loss(&auth_weights, &prov_entropies, 2.7, "mean").unwrap();
    let mean_loss: f32 = result.item();

    assert!(mean_loss > 0.0, "Batch mean loss should be positive: {}", mean_loss);
}
