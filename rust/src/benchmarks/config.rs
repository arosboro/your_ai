...
pub static BENCHMARK_REGISTRY: Lazy<HashMap<String, BenchmarkConfig>> = Lazy::new(|| {
    ...
    registry.insert("truthfulqa".to_string(), BenchmarkConfig {
    ...
    registry.insert("censorbench".to_string(), BenchmarkConfig {
...
pub fn get_benchmark_config(name: &str) -> anyhow::Result<&BenchmarkConfig> {
    BENCHMARK_REGISTRY.get(name)
        .ok_or_else(|| anyhow::anyhow!("Unknown benchmark: {}. Available: {:?}", name, BENCHMARK_REGISTRY.keys().collect::<Vec<_>>()))
}
